...
from src.core.processing.two_social_video_processing import process_two_social_files
...
def show():
    """显示两鼠社交分析页面 / Display two-social test analysis page"""
    # 加载样式
    load_custom_css()

    # 页面标题和说明
    st.title("🐁 两鼠社交分析 / Two-Social Test Analysis")
    ...
    # 设置路径和时间
    root_directory = os.path.join(get_data_path(), "two-social")
    current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    web_log_file_path = os.path.join(get_root_path(), "logs", "usage.txt")

    # 设置模型
    models = {
        "ICR-DJ-Two-Social-V2": os.path.join(
            get_models_path(),
            "ABmodels/DJ-two-social-500px-narrow-2024-05-13/config.yaml",
        ),
        "C57-DJ-Two-Social-V2": os.path.join(get_models_path(), "ABmodels/C57-Two-Social-T2-2024-05-15/config.yaml"),
    }
        ...
        if folder_path:
            ...
            if st.button("⚡ 处理分析结果 / Process Analysis Results", use_container_width=True):
                with st.spinner("处理中 / Processing..."):
                    process_two_social_files(folder_path, 0.999, 15, 35)
