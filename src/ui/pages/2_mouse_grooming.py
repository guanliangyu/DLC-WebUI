...
def show():
    """显示小鼠理毛行为分析页面 / Display mouse grooming behavior analysis page"""
    # 加载样式
    load_custom_css()

    # 页面标题和说明
    st.title("🐁 小鼠理毛行为分析 / Mouse Grooming Behavior Analysis")
    ...
    # 设置路径和时间
    root_directory = os.path.join(get_data_path(), "grooming")
    current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    web_log_file_path = os.path.join(get_root_path(), "logs", "usage.txt")

    # 设置模型
    models = {
        "ICR-DJ-Grooming-V2": os.path.join(
            get_models_path(),
            "ABmodels/DJ-grooming-500px-narrow-2024-05-13/config.yaml",
        ),
        "C57-DJ-Grooming-V2": os.path.join(get_models_path(), "ABmodels/C57-Grooming-T2-2024-05-15/config.yaml"),
    }
