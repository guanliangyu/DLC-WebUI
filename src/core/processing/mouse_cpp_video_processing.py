...
def process_mouse_cpp_video(file_path, folder_path, confidence_threshold=0.9):
    ...
    try:
        ...
        # 数据清理：根据置信度过滤数据点
        data.loc[data[('bodypart1', 'likelihood')] <= confidence_threshold, [('bodypart1', 'x'), ('bodypart1', 'y')]] = np.nan
        ...
        # 绘制轨迹
        plt.subplot(2, 1, 1)
        plt.plot(x_smoothed, y_smoothed, 'b-', alpha=0.5, label='运动轨迹 / Movement Trajectory')
        plt.axvline(x=x_boundaries[0], color='r', linestyle='--', label='左边界 / Left Boundary')
        plt.axvline(x=x_boundaries[1], color='g', linestyle='--', label='右边界 / Right Boundary')
        ...
        total_data = {
            ...
            '中区域总时间 / Total Middle Time (s)': np.sum((x_smoothed >= x_boundaries[0]) & (x_smoothed < x_boundaries[1])) / frame_rate,
            ...
            '中区域偏好指数 / Middle Preference Index': (np.sum((x_smoothed >= x_boundaries[0]) & (x_smoothed < x_boundaries[1])) / total_frames) * 100,
