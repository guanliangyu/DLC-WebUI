...
def process_mouse_tc_video(file_path, folder_path, confidence_threshold=0.9):
    ...
    try:
        ...
        plt.axhline(y=-y_boundaries[0], color='r', linestyle='--', label='上边界 / Upper Boundary')
        plt.axhline(y=-y_boundaries[1], color='g', linestyle='--', label='下边界 / Lower Boundary')
        ...
        total_data = {
            ...
            '中区域总时间 / Total Middle Time (s)': np.sum((y_smoothed >= y_boundaries[0]) & (y_smoothed < y_boundaries[1])) / frame_rate,
