...
with st.expander("💡 使用说明 / Instructions", expanded=True):
    ...
    - 视频方向：俯视，两只小鼠在同一视野内 / Video Direction: Top-down view, both mice in the same field of view
    """
    )
...
# 设置模型
models = {
    "C57-Social-V1": os.path.join(
        get_models_path(), "Social-mouse-C57-Trial-3-2024-04-28/config.yaml"
    ),
    "C57-Social-V2": os.path.join(
        get_models_path(), "Social-mouse-C57l-trial-2-2024-04-27/config.yaml"
    ),
    "C57-Social-V3": os.path.join(
        get_models_path(), "Social-mouse-C57-trial-1-2024-03-09/config.yaml"
    ),
}
